...
//#region src/kernel/dependency.ts
let globalActiveComputation = null;
...
function trackDependency(signal) {
	if (globalActiveComputation) signal.subscribe(globalActiveComputation);
}
//#endregion
//#region src/kernel/path.ts
...
function path(...parts) {
	...
	for (const part of parts) {
		...
		if (str === "*") {
			if (globType === "deep") throw new Error("Cannot use * after ** in path");
			globType = "shallow";
			segments.push("*");
		} else if (str === "**") {
			if (globType === "shallow") throw new Error("Cannot use ** after * in path");
			globType = "deep";
			segments.push("**");
		} else if (str === "") throw new Error("Path segment cannot be empty");
		else {
			if (str.includes("/")) throw new Error("Path segment cannot contain '/'");
			segments.push(str);
		}
	}
	return {
...
function matchesPath(pattern, target) {
	if (pattern.globType === "none") return pattern.raw === target.raw;
	if (pattern.globType === "deep") {
		...
		function match(pi, ti) {
			...
			const pSeg = p[pi];
			if (pSeg === "*") return match(pi + 1, ti + 1);
			if (pSeg === "**") {
				if (match(pi + 1, ti)) return true;
				return match(pi, ti + 1);
			}
			if (pSeg === t[ti]) return match(pi + 1, ti + 1);
			return false;
		}
		return match(0, 0);
	}
	if (pattern.globType === "shallow") {
		...
		for (let i = 0; i < pattern.segments.length; i++) {
			...
			if (pSeg !== "*" && pSeg !== target.segments[i]) return false;
		}
		return true;
	}
	return false;
...
function getParentPath(p) {
	...
	if (nonGlobSegments.length === 0) return null;
	if (nonGlobSegments.length === 1) return path(...nonGlobSegments);
	return path(...nonGlobSegments.slice(0, -1));
}
function joinPath(base, ...parts) {
	return path(...base.segments.filter((s) => s !== "*" && s !== "**"), ...parts);
}
//#endregion
...
//#endregion
//#region src/kernel/config.ts
let currentConfig = {
	lockViolation: "throw",
	satori: Ie({ logLevel: "debug" })
};
function configureSairin(config) {
	if (config.lockViolation !== void 0) currentConfig.lockViolation = config.lockViolation;
	if (config.satori !== void 0) currentConfig.satori = config.satori;
}
...
function getNodesUnder(prefix) {
	...
	for (const node of nodeRegistry.values()) {
		...
		if (isExactMatch || hasProperPrefix || matchesPath(prefix, node.path)) result.push(node);
	}
	return result;
}
...
function watch(pattern, callback) {
	...
	return () => {
		for (const [node, handlers] of nodeHandlers) for (const handler of handlers) node.subscribers.delete(handler);
		nodeHandlers.clear();
	};
}
...
const lockedPaths = /* @__PURE__ */ new Map();
function lock(path, options) {
	const key = path.raw;
	lockedPaths.set(key, {
		owner: options.owner,
...
function handleLockViolation(path, owner, attemptedOwner) {
...
function assertLock(path, owner, attemptedOwner) {
...
function scheduleIncrementalCleanup(nodes, options = {}) {
	...
	const cleanupChunk = () => {
		...
		if (cleanupIndex < scheduledCleanupNodes.length) {
			...
			if (elapsed > CLEANUP_WARN_THRESHOLD_MS) logger.warn(`Incremental cleanup falling behind: ${elapsed}ms elapsed`, { tags: ["memory", "gc"] });
...
//#endregion
//#region src/kernel/signal.ts
var Signal = class {
	id;
	path;
	_node;
	constructor(path, initial, forceSet = false) {
		this.id = parseInt(generateUniqueId(), 36);
		this.path = path;
		this._node = getOrCreateNode(path, "signal");
...
function signal(pathOrInitial, initial) {
	if (isPathKey(pathOrInitial)) {
		let path = pathOrInitial;
		const resolved = resolveAlias(path);
		if (resolved) path = resolved;
		const existingNode = getNode(path);
		if (existingNode && existingNode.kind === "signal") return new Signal(path, existingNode.value, false);
		return new Signal(path, initial, true);
	}
...
var Derived = class {
	...
	constructor(path, fn, options = {}) {
		this.id = parseInt(generateUniqueId(), 36);
		this.path = path;
		this._node = getOrCreateNode(path, "derived");
...
function derived(path, fn, options) {
	return new Derived(path, fn, options);
}
//#endregion
//#region src/kernel/batch.ts
let pendingEffects = /* @__PURE__ */ new Set();
...
function createEffect$1(fn, schedule) {
	let cleanupFn;
	let disposed = false;
	const logger = getSairinLogger();
...
const effect = (fn) => createEffect$1(fn, scheduleEffect);
const effectSync = (fn) => createEffect$1(fn, (r) => r());
const effectIdle = (fn) => createEffect$1(fn, (runner) => {
	if (typeof requestIdleCallback !== "undefined") requestIdleCallback(() => runner());
...
function createSignal(path, value) {
	return new Signal(path, value);
}
function createMemo(path, fn, options) {
	return new Derived(path, fn, options);
}
...
function reactive(obj, basePath) {
	...
	for (const key of Object.keys(obj)) {
		...
		} else if (isObject(value)) result[key] = reactive(value, path(storePath, String(key)));
		else result[key] = signal(path(storePath, String(key)), value);
	}
	result.$ = signal$;
	result.$raw = obj;
	return new Proxy(result, {
		...
		set(target, prop, newValue) {
			if (prop === "$" || prop === "$raw") throw new Error("Cannot set $ or $raw directly");
			const existingProp = target[prop];
			if (existingProp && existingProp instanceof Signal) existingProp.set(newValue);
			else if (isObject(newValue) && newValue !== null) {
				const parentSegments = target.$.path.segments;
				target[prop] = reactive(newValue, path(...parentSegments, String(prop)));
			} else {
				const parentSegments = target.$.path.segments;
...
function setReactive(reactiveObj, value) {
	for (const key of Object.keys(value)) {
		...
		if (propSignal && propSignal instanceof Signal) propSignal.set(value[key]);
	}
	reactiveObj.$.set(value);
}
//#endregion
//#region src/store/array.ts
let arrayId = 0;
...
var ReactiveArray = class {
	...
	constructor(initial = []) {
		...
		this.lengthSignal = signal(path("array", this.id, "length"), initial.length);
	}
	notify() {
	...
	get(index) {
		return this.itemsSignal.get()[index];
	}
	set(index, value) {
...
var ReactiveMap = class {
	...
	removeKeyId(key) {
		if ((typeof key === "object" || typeof key === "function") && key !== null) this.keyIds.delete(key);
		else {
			const prim = key;
	...
	clear() {
		for (const key of this.entries.keys()) this.removeKeyId(key);
		this.entries.clear();
	...
	values() {
		this.sizeSignal.get();
		function* gen() {
			for (const sig of this.entries.values()) yield sig.get();
		}
	...
	entriesIterable() {
		this.sizeSignal.get();
		function* gen() {
			for (const [key, sig] of this.entries.entries()) yield [key, sig.get()];
		}
		return gen.call(this);
	}
	forEach(fn) {
		this.sizeSignal.get();
		for (const [key, sig] of this.entries.entries()) fn(sig.get(), key, this);
	}
	toArray() {
		this.sizeSignal.get();
		const result = [];
		for (const [key, sig] of this.entries.entries()) result.push([key, sig.get()]);
		return result;
	}
	subscribe(fn) {
		this.subscribers.add(fn);
			...
			*/
			...
			const perform = async () => {
				try {
					...
					if (!runController.signal.aborted && abortController === runController) result.set(data);
				} catch (e) {
					if (!runController.signal.aborted && abortController === runController) error.set(e instanceof Error ? e : new Error(String(e)));
				} finally {
					if (abortController === runController) running.set(false);
				}
			};
...
function parallel(...fns) {
	...
	return {
		...
		start: async () => {
			...
			const allResults = [];
			const allErrors = [];
			for (const res of settled) if (res.status === "fulfilled") allResults.push(res.value);
			else {
	...
	*/
	...
	return {
		...
		start: async () => {
			...
			running.set(true);
			error.set(null);
			const promises = fns.map((fn, index) => fn(runController.signal).then((value) => ({
				index,
				value
			})).catch((e) => ({
				index,
			...
			if ("error" in outcome) error.set(outcome.error);
			else {
...
function resource(loader, initialValue = null) {
	...
	const load = () => {
		...
		error.set(null);
		loader().then((data) => {
			if (!currentController.signal.aborted && abortController === currentController) value.set(data);
		}).catch((e) => {
			if (!currentController.signal.aborted && abortController === currentController) error.set(e instanceof Error ? e : new Error(String(e)));
		}).finally(() => {
			if (abortController === currentController) loading.set(false);
		});
	};
	load();
	return {
...
function resourceWithSignal(source, initialValue = null) {
	...
	const load = (loader) => {
		...
		error.set(null);
		loader().then((data) => {
			if (!currentController.signal.aborted && abortController === currentController && currentLoader === loader) value.set(data);
		}).catch((e) => {
			if (!currentController.signal.aborted && abortController === currentController && currentLoader === loader) error.set(e instanceof Error ? e : new Error(String(e)));
		}).finally(() => {
			if (abortController === currentController && currentLoader === loader) loading.set(false);
		});
	};
	effect(() => {
...
var SuspenseBoundary = class {
	...
	error = signal(path("suspense", this.id, "error"), null);
	fallback;
	constructor(config) {
...
//#endregion
//#region src/async/transition.ts
...
function useTransition(timeout = 0) {
	const pending = signal(path("transition", "pending", generateUniqueId()), false);
	const start = (fn) => {
		pending.set(true);
		...
		if (timeout > 0) setTimeout(performTransition, timeout);
		else queueMicrotask(performTransition);
	};
	return {
		pending,
		start
...
function deferred(value, options = {}) {
	...
	const deferredValue = signal(path("transition", "deferred", generateUniqueId()), value.peek());
	return {
		signal: deferredValue,
		dispose: effect(() => {
			const newValue = value.get();
			if (equals(deferredValue.peek(), newValue)) return;
			if (timeoutMs > 0) setTimeout(() => {
				if (!equals(deferredValue.peek(), newValue)) deferredValue.set(newValue);
			}, timeoutMs);
			else queueMicrotask(() => {
				if (!equals(deferredValue.peek(), newValue)) deferredValue.set(newValue);
			});
...
function useDeferredValue(value, timeoutMs = 0) {
	...
	const sig = signal(path("transition", "deferredValue", generateUniqueId()), value);
	let deferredSignal;
	effectSync(() => {
...
function deferValue(value, timeoutMs) {
	return useDeferredValue(new Signal(path("async", "deferred", generateUniqueId()), value), timeoutMs);
}
//#endregion
...
function createContext(defaultValue, name) {
	...
	return {
		...
		consume: () => {
			const stack = contextStacks.get(contextId);
			if (!stack || stack.length === 0) return defaultValue;
...
function notifySignalCreated(signal, name) {
	debugHooks.onSignalCreated?.(signal, name);
}
function notifySignalRead(signal) {
	debugHooks.onSignalRead?.(signal);
}
function notifySignalWritten(signal, oldValue, newValue) {
	debugHooks.onSignalWritten?.(signal, oldValue, newValue);
}
function notifyEffectCreated(effect) {
	debugHooks.onEffectCreated?.(effect);
}
function notifyEffectRun(effect) {
	debugHooks.onEffectRun?.(effect);
}
function notifyDerivedInvalidated(derived) {
	debugHooks.onDerivedInvalidated?.(derived);
}
const registeredSignals = /* @__PURE__ */ new Map();
const registeredEffects = /* @__PURE__ */ new Map();
const registeredDerived = /* @__PURE__ */ new Map();
function registerSignal(signal) {
	registeredSignals.set(signal.id, signal);
}
function registerEffect(effect, id) {
	registeredEffects.set(id, effect);
}
function registerDerived(derived) {
...
exports.SuspenseBoundary = SuspenseBoundary;
exports.__resetBatchForTesting = __resetBatchForTesting;
exports.__resetRegistryForTesting = __resetRegistryForTesting;
exports.alias = alias;
exports.assertLock = assertLock;
exports.batch = batch;
exports.batched = batched;
exports.bindAttribute = bindAttribute;
exports.bindBooleanAttribute = bindBooleanAttribute;
exports.bindClass = bindClass;
exports.bindDisabled = bindDisabled;
exports.bindElementSignal = bindElementSignal;
exports.bindEvent = bindEvent;
exports.bindHtml = bindHtml;
exports.bindInputChecked = bindInputChecked;
exports.bindInputValue = bindInputValue;
exports.bindProperty = bindProperty;
exports.bindSelectValue = bindSelectValue;
exports.bindStyle = bindStyle;
exports.bindText = bindText;
exports.bindVisibility = bindVisibility;
exports.capRetainedMemory = capRetainedMemory;
exports.captureGraph = captureGraph;
exports.checkLock = checkLock;
exports.clearGraph = clearGraph;
exports.configureSairin = configureSairin;
exports.createBinding = createBinding;
exports.createContext = createContext;
exports.createContextWithOptions = createContextWithOptions;
exports.createEffect = createEffect;
exports.createEffectSync = createEffectSync;
exports.createList = createList;
exports.createMap = createMap;
exports.createMemo = createMemo;
exports.createPath = path;
exports.createResource = createResource;
exports.createResourceFromSignal = createResourceFromSignal;
exports.createSignal = createSignal;
exports.createStore = createStore;
exports.deferValue = deferValue;
exports.deferred = deferred;
exports.deleteNode = deleteNode;
exports.derived = derived;
exports.disableDebug = disableDebug;
exports.effect = effect;
exports.effectIdle = effectIdle;
exports.effectSync = effectSync;
exports.enableDebug = enableDebug;
exports.flow = flow;
exports.generateId = generateId;
exports.generateUniqueId = generateUniqueId;
exports.getAllNodes = getAllNodes;
exports.getDebugHooks = getDebugHooks;
exports.getGlobalActiveComputation = getGlobalActiveComputation;
exports.getIsTransition = getIsTransition;
exports.getNode = getNode;
exports.getNodesUnder = getNodesUnder;
exports.getOrCreateNode = getOrCreateNode;
exports.getParentPath = getParentPath;
exports.getSairinConfig = getSairinConfig;
exports.hasNode = hasNode;
exports.hasPendingEffects = hasPendingEffects;
exports.isAlias = isAlias;
exports.isFlushing = isFlushing;
exports.isLocked = isLocked;
exports.isPathKey = isPathKey;
exports.isReactive = isReactive;
exports.isSignal = isSignal;
exports.joinPath = joinPath;
exports.lock = lock;
exports.matchesPath = matchesPath;
exports.notifyDerivedInvalidated = notifyDerivedInvalidated;
exports.notifyEffectCreated = notifyEffectCreated;
exports.notifyEffectRun = notifyEffectRun;
exports.notifySignalCreated = notifySignalCreated;
exports.notifySignalRead = notifySignalRead;
exports.notifySignalWritten = notifySignalWritten;
...
exports.reactiveArray = reactiveArray;
exports.reactiveMap = reactiveMap;
exports.registerDerived = registerDerived;
exports.registerEffect = registerEffect;
exports.registerSignal = registerSignal;
exports.resolveAlias = resolveAlias;
exports.resource = resource;
exports.resourceWithSignal = resourceWithSignal;
exports.scheduleEffect = scheduleEffect;
exports.scheduleIncrementalCleanup = scheduleIncrementalCleanup;
exports.sequence = sequence;
exports.setGlobalActiveComputation = setGlobalActiveComputation;
...
exports.useTransitionResult = useTransitionResult;
exports.watch = watch;
