export { resource, resourceWithSignal, SuspenseBoundary, type Resource, type SuspenseConfig } from './resource';
export { 
  startTransition, 
  getIsTransition, 
  useTransition, 
...
import { useTransition, useDeferredValue, type TransitionResult } from './transition';
import { Signal } from '../kernel/signal';

export function createResource<T>(loader: () => Promise<T>, initialValue?: T): Resource<T> {
...
export function deferValue<T>(value: T, timeoutMs?: number): ReturnType<typeof useDeferredValue> {
